...
import studentsInfo from '../static/students'
...
const PersonDetailTabs = defineComponent({
  props: {
    userInfo: {
      require: true,
      type: Object as PropType<DetailTabsProps>
    }
  },
  setup(props) {
    const activeKey = ref<string>('1')
    const paperDetailVisible = ref<boolean>(false)
    const currentPaperDetail = ref<PapersParams>({} as PapersParams)

    const handlePaperDetailClick = (item: PapersParams) => {
    ...
    const students = computed(() => {
      return studentsInfo.filter(item => {
        return props.userInfo?.students.includes(item.id) ?? false;
      })
    })
        ...
        <div>
          ...
          >
            <a-tab-pane key="1" tab="个人简介" class="px-4 py-2">
              ...
              {props.userInfo?.description?.split('\n').map((item) => {
            ...
            <a-tab-pane key="2" tab="奖项荣誉" class="px-4 py-2">
              <a-timeline>
                {props.userInfo?.honors?.map((item) => {
                  return (
            ...
            <a-tab-pane key="3" tab="发表论文" class="px-4 py-2">
              <a-timeline>
                {props.userInfo?.papers?.map((item) => {
                  return (
                    <a-timeline-item onClick={() => handlePaperDetailClick(item)}>
                      <div class="cursor-pointer hover:text-slate-800">{`${dayjs(item.time).format(
                        'YYYY'
                      )} - ${item.title}`}</div>
                    </a-timeline-item>
                  )
            ...
            <a-timeline>
                {props.userInfo?.projects?.map((item) => {
                  return (
            ...
            <a-tab-pane key="5" tab="学生信息" class="px-4 py-2">
              <div class="flex flex-wrap">
              {
                students.value.map(item => {
                  return (
                    <StudentDetailCard studentInfo={item}></StudentDetailCard>
                  )
                })
              }
              </div>
